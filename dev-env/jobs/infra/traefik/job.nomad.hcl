  ...
  group "traefik" {
    ...
    task "traefik" {
      ...
      template {
        ...
        # postgres
        TRAEFIK_ENTRYPOINTS_POSTGRES_ADDRESS=":5433"
        TRAEFIK_ENTRYPOINTS_POSTGRES_PROXYPROTOCOL_TRUSTEDIPS="${local.cidr}"
        TRAEFIK_ENTRYPOINTS_POSTGRES_TRANSPORT_LIFECYCLE_REQUESTACCEPTGRACETIMEOUT="15"
        TRAEFIK_ENTRYPOINTS_POSTGRES_TRANSPORT_LIFECYCLE_GRACETIMEOUT="10"
        EOH
      ...
      template {
        ...
        TRAEFIK_PROVIDERS_CONSULCATALOG_ENDPOINT_DATACENTER="${var.workspace}"
